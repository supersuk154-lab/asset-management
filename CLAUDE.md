...
```text
G:\내 드라이브\자산관리 자동화\
├── CLAUDE.md                     ← 오케스트레이터 메인 지침서 (현재 파일)
├── .claude\agents\               ← 7개의 에이전트 프롬프트
│   ├── kyc-collector.md
│   ├── macro-analyst.md
...
1. kyc-collector
   ...
   (v5.3 선택 컬럼 4종 — 목표금액·대출금리·보장성보험·예상연금월액 — 은 하위호환: 없거나 빈칸이면 kyc-collector가 null 처리 후 계속 진행. Funded Ratio·보장공백·은퇴인출 설계 고도화에 사용)
...
5. risk-scorer
   ...
   [schemas.py: RiskScorerOutput → 점수 범위·등급 이모지 강제]
        ↓
6. report-writer
   입력: compliance_rules.json (면책 고지 문구 = compliance_warnings.full_disclaimer 값 그대로 / Easy 리포트용 easy_mode_glossary)
   출력: data/clients/{id}/reports/{date}.md + .html  (정식 리포트)
         + data/clients/{id}/reports/{date}_easy.md + _easy.html  (초보자용 1페이지 Easy 리포트 — 10단계, 필수)
   [Easy 리포트] 정식 리포트와 동일 소스 JSON으로 생성하되, easy_mode_glossary 비유 사전·숫자 원화 번역·진단→이유→행동 3박자로 초보자 가독성 최적화. 숫자는 정식 리포트와 1원도 다르면 안 됨(reviewer V12/V30 검사).
...
**Validation 에러 발생 시:** validate 출력의 `── Claude에게 전달할 수정 요청 ──` 블록을 해당 에이전트에게 그대로 주입하여 재작성 요청 → 저장 → validate 재실행 (최대 3회)
...
- [x] Phase 1: 기초 인프라 — 폴더 구조, CLAUDE.md
...
- [x] Phase 5-A: Method B 반자동화 — orchestrator.py CLI 구현 완료
  ...
  - Pydantic 검증 에러 메시지 → Claude 자가 수정 루프 연결
...
- [x] Phase 5-D: 재무설계 방법론 고도화 (보완방안 반영)
  - **MDD 한도 2차원화:** 시간지평 × capacity 테이블 + 'MDD-배분 정합성 클램핑'으로 글라이드패스와 IPS 손실한도 모순 제거 (ips_limit_breach 자동 방지)
  - **안전버킷 듀레이션 상한:** 초장기채(20·30년물) ETF 안전버킷 편입 금지 (compliance_rules.duration_cap, reviewer V11 확장)
  - **목표 자금 적정성(Funded Ratio):** goal_amount_krw 수집 → portfolio-designer가 target_multiple 역산 + 3택 처방
  - **꼬리위험 비중 단일기준화:** '전체 자산 5~15%' 기준 통일 + 내부비중 파생식 (reviewer V28)
  - **행동갭 지표 v2:** 공격성 채점 → Willingness-Capacity '일치도(gap)' 채점으로 전환 (고령 안정형 왜곡 해소)
  - **신규 폼 4종(v5.3, 하위호환):** 목표금액·대출금리·보장성보험·예상연금월액 → Funded Ratio·보장공백(insurance_gap, V29)·은퇴인출 설계
  - **세제 개인화:** 서민형 ISA·세액공제 소득차등·증여세·건강보험료 임계값 (compliance_rules 확장)
  - **인적자본 NPV 정정:** 무할인 단순곱 → 연금현가계수(r=3%) 적용
  - **동일원인 1회차감 원칙:** 경험없는 적극형 중복 감점 방지 (portfolio-designer·risk-scorer·reviewer V22)
- [x] Phase 5-E: 초보자용 Easy 리포트 (가독성 개선)
  - `templates/master_report_easy.md` / `.html` 신규 — 1페이지 "내 돈 사용설명서"
  - report-writer 10단계 추가: 정식 리포트와 별도로 `{date}_easy.md/.html` 생성 (동일 소스 JSON, 숫자 1원도 불일치 금지)
  - `compliance_rules.json.easy_mode_glossary` — 비유 사전·톤 규칙·점수 비유·미니사전 단일 진실 공급원
  - 핵심: 전문용어 0 + 모든 %를 원화로 번역 + 진단→이유→행동 3박자 + "왜요?" Q&A + "떨어지면 어떡하죠" 선제 대응 + 체크박스 실행 카드
  - orchestrator `_validate_report` 확장(easy 파일 미치환 검사·날짜탐지 제외), reviewer V5/V12 확장 + V30 신설
- [ ] Phase 5-C: 완전 자동화 — Claude API 연동 (call_agent_with_validation 함수 기구현, 추후 배선)
